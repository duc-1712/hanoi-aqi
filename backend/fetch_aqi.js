    ...
    if (aqi !== null && !isNaN(aqi)) {
      ...
      await pool.query(
        `INSERT INTO station_history
         (station_id, aqi, pm25, pm10, o3, no2, so2, co, recorded_at)
      ...
      console.log(
        `${source.padEnd(6)} | ${station.name.padEnd(28)} | AQI: ${String(
          aqi,
        ).padStart(3)} | OK`,
      );
      ...
      console.log(
        `${source.padEnd(6)} | ${station.name.padEnd(28)} | Không có dữ liệu`,
      );
      return false;
    }
  } catch (err) {
...
export async function updateAQIData() {
  ...
  if (!AQICN_TOKEN && !IQAIR_KEY) {
    console.error(`[${timeStr}] Thiếu Token!`);
    return;
  }
  ...
  const results = await Promise.all(
    allStations.map((station) => processStation(station)),
  );
  ...
  console.log(`\n=== HOÀN TẤT: ${success}/${allStations.length} trạm ===\n`);
}
